...
namespace Unity.U2D.AI.Editor
{
    ...
    internal class Painter : IDisposable
    {
        ...
        class PaintMaterial : IDisposable
        {
            ...
            {
                material = new Material(Utilities.LoadPackageResource<Shader>("shaders/Painter.shader")) { hideFlags = HideFlags.HideAndDontSave };
            }
        ...
        class TransformMaterial : IDisposable
        {
            ...
            public readonly Material material = new(Utilities.LoadPackageResource<Shader>("shaders/DoodleTransform.shader")) { hideFlags = HideFlags.HideAndDontSave };
        ...
        public void InitializeWithData(byte[] imageData)
        {
            if (m_Texture == null)
            {
                m_Texture = new Texture2D(m_Size.x, m_Size.y)
                {
                    ...
                    name = "com.unity.2d.enhancer Painter.InitializeWithData"
                };
            }
            ...
            else
            {
                ...
                Array.Fill(pixels, clearColor, 0, pixels.Length);
            ...
            m_RenderTexture = GetNewRenderTexture(m_Size.x, m_Size.y, clearColor, "2DMuse-Painter-RT");
            UpdateRenderTexture();
        ...
        public void Resize(Vector2Int newSize)
        {
            ...
            if (m_Texture == null)
            {
                ...
                {
                    hideFlags = HideFlags.HideAndDontSave,
                    name = "com.unity.2d.enhancer Painter.Resize"
                };
            }
            else
                m_Texture.Reinitialize(m_Size.x, m_Size.y);
        ...
        {
            Paint(startPosition, endPosition, paintColor, brushRadius);
        }
        ...
        {
            Paint(startPosition, endPosition, clearColor, brushRadius);
        }
        ...
        /// <param name="radius">Paint radius.</param>
        public void Paint(Vector2 startPosition, Vector2 endPosition, Color color, float radius)
        {
            if (!isInitialized)
                return;
            ...
            m_PaintMaterial.SetRadius(radius);
        ...
        public void DoodleArea(Vector2[] area)
        {
            if (!isInitialized)
                return;

            if (FillArea.useComputeShader)
            {
                FillArea.FillTextureCompute(m_RenderTexture, area, m_Size, maskColor);
            }
            else
            {
                FillArea.FillTextureJob(m_Texture, area, m_Size, maskColor);
                UpdateRenderTexture();
            }
        }
        ...
        static RenderTexture GetNewRenderTexture(int width, int height, Color clearColor, string name)
        {
